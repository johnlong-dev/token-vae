...
| Perturbation Stability | Pass | 0.8994 | 0.5000 |
...
- Unique token fraction: 56.19%
- Unique tokens decoded: 5619 / 10000
- Max token frequency: 0.18%
- Tokens seen once: 35.30%
- Prior entropy (mean/median/max): 4.08 / 4.32 / 6.21
- log(V): 9.68
- Gini coefficient: 0.7675
...
- Margin sensitivity (|Δ(top1−top2)|/||ε||): p50=1.58e-01, p90=4.09e-01, p99=6.02e-01, max=6.52e-01

Off-manifold robustness sweep:

- σ=0.5: safe=100.00%, unique=100.00%, H_mean=0.05, H_max=2.12, top1_retention=100.00%
- σ=1.0: safe=100.00%, unique=100.00%, H_mean=0.93, H_max=5.13, top1_retention=87.00%
- σ=2.0: safe=100.00%, unique=99.00%, H_mean=1.27, H_max=3.08, top1_retention=12.50%
...
- Max JS distance: 0.4073
- Mean JS divergence (derived): 0.0054
- Max JS divergence (derived): 0.1659
- Mean entropy: 0.74
- Max entropy: 6.81
...
- Midpoint entropy mean: 3.19
- Mean argmax switch rate: 0.0526
- Mean top-k Jaccard: 0.8751
...
- Entropy vs prior (mean Δ / max Δ): -3.34 / 0.60
- Entropy vs prior divergence flagged: Yes

Max JS divergence case:
```
  after (6575) → ▁manually (14790), steps 8→9, alpha 0.42→0.47, JSdist=0.4073, norm 5.43→5.51
  top-10 from:
    after (6575): 0.603
    ▁manually (14790): 0.295
    ▁in (35): 0.001
    ▁to (32): 0.001
    ▁a (6): 0.001
    . (15914): 0.001
...
```
  ▁Col (1193) → res (157), step 10, alpha=0.53, H=6.81, norm=3.75
  ▁Other (2541) → gers (3116), step 10, alpha=0.53, H=6.72, norm=4.05
  ▁Other (2541) → gers (3116), step 9, alpha=0.47, H=6.62, norm=4.03
  ▁networking (8888) → ▁oper (1003), step 10, alpha=0.53, H=6.36, norm=4.45
  ▁Col (1193) → res (157), step 11, alpha=0.58, H=6.34, norm=3.73
```

Distance bucket metrics (train pairs):
```
  thresholds: near<= 8.927, medium<= 9.667
  near: mean_js_dist=0.078, max_js_dist=0.302, mean_H=1.05, max_H=6.81, flip=1.00, switch=0.053, jaccard=0.906, cliff=0.53, plateau=0.95, n=17
  medium: mean_js_dist=0.071, max_js_dist=0.356, mean_H=0.62, max_H=5.84, flip=1.00, switch=0.053, jaccard=0.872, cliff=1.50, plateau=0.95, n=16
  far: mean_js_dist=0.071, max_js_dist=0.407, mean_H=0.55, max_H=6.36, flip=1.00, switch=0.053, jaccard=0.847, cliff=1.76, plateau=0.95, n=17
```
...
- Max mean entropy: 4.21
- Mean mean entropy: 4.09
- Min unique fraction: 96.00% (threshold 10.00%)
- Mean step change rate: 19.76% (threshold 1.00%)
- Walks: 100, Steps: 50
...
```
  step  non_special  entropy  unique    norm
     0      100.00%     4.21      99   16.14
    10      100.00%     4.09      99   16.16
    20      100.00%     4.07      99   16.11
    30      100.00%     3.99      98   16.16
    40      100.00%     4.16     100   16.17
```
